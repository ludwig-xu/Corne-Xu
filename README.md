<img width="1280" height="960" alt="小配列入坑：分体+蓝牙+小配列 记录小而美_1_须公子_来自小红书网页版" src="https://github.com/user-attachments/assets/c857ae86-3cdd-4942-8bc6-defc4397d426" />
- `202404第一把Corne带墨水屏 `
<img width="5712" height="4284" alt="黑corne" src="https://github.com/user-attachments/assets/496a00b5-0e99-40a8-9110-288636d4a66a" />
- `202604 2年后，Corne带旋钮+方向键+Oled`
...
- 我自己对键位排布的认知其实也是渐进明细的,从一开始看Dactyl-Manuform觉得键盘就该长这样,到理解小配列键盘,到较为激进的3x5配列狂热信徒,再到回归折中的3x6配列,还是一个质疑-理解-成为,最后返璞归真的过程.
...
![miryoku-kle-reference](https://github.com/ludwig-xu/Corne-Xu/assets/167788996/c76b136d-dc05-4c9b-a5f7-a36329dd0812)
...
- 除此之外,从别的大神那里汇总的技巧或者原则如下:
	- 习惯原则: 传统键盘用右手实现倒T布局的上下左右,FPS玩家对左手wasd接收度也极高。我们可以在切层后在左手的esdf也布置上下左右;传统键盘我们在小键盘上用九宫格布局实现数字输入,我们可以在切层后在右手同样布局数字输入.
...
- 总之,3x6配列就是极端方案往回退一步,尽量沿用传统键盘的布局,在不能忍的地方,愿意改的地方,用更合理的布局;在能负担能接受的地方维持传统.加一句:在不确定的地方冗余布局,手指会告诉你哪个方案好的.
	...
	3.我的Keymap
- 一共5层,还是比较克制的.base layer是QWER布局的默认层;num layer是实现数字和数字有关的加减乘除括号等的输入以及蓝牙相关。corne可以支持5个蓝牙设备，只有这层塞起来比较从容;nav layer实现光标移动以及相关的文档操作按键;media layer实现Fn,媒体控制,键盘重启,bootload以及RGB控制键;最后加入的是鼠标层，模拟鼠标移动，可以不用，但不能没有。
- Base layer:
- <img width="1239" height="372" alt="base" src="https://github.com/user-attachments/assets/7a57b79f-9e0f-43ed-80e9-df25cf03595b" />
...
- 编辑于202604：哈哈哈哈，这段真是整段垮掉！但是就像我说的，layout是靠用出来的。当时想的再美，在使用过程中你的手会告诉你对还是不对，要坚持还是要改善。首先，nav层和num层确实在抢左右手！最终现在用下来是num层抢赢了右手。nav层委屈的占据了左手区，而且，左手区的倒T布局一点儿都不违和。什么效率，在从小打CS的中登手里，左手倒T就是效率！这是不是意味着左下右上的一字长蛇阵就不行了呢？又到了成年人我全都要的场景。左下右上就应该留在右手！这下既保留了双手操作时高效的左手倒t，又使右手具备了单手控方向的能力。其余边角部分的逻辑还是能跑通的。倒T下面保留编辑按钮，右手homerow modifer方便操作。
- num layer:
- <img width="1242" height="357" alt="num" src="https://github.com/user-attachments/assets/8554c69b-cd98-4e89-8776-eb7402b0bc31" />
- 先说一下为什么不能选择一字长蛇阵来输入数字.我个人的体验,一字长蛇阵反人类.尤其是输入身份证,电话号码这样的长数字的时候,传统键盘拿小键盘输入数字的效率远高于一字长蛇阵的效率.在本身就需要切层使用的前提下,不要给选择的机会,就确认下来左手(或右手)打数字是最快的.除了纯打数字,拼音选字也是高频工况,那下翻页上翻页就要放在同层好按的位置.在当前联想输入强大的背景下,大多数你想输入的字都会出现在123,不出现的情况,基本就得翻页,于是=放在3右边,-放在=上边.那回到纯数字工况,-=都有了,上面补一个+很合理吧?那加减有了,对角侧补上乘除很合理吧?边角的地方再补上点号,传统位置补0,~~右手~~ 左手补上[](),~~右手~~ 左手再补冗余homerow mod,基本在数字层需要的键位就全了.有个特殊的点,~~右手~~ 左手第一列第三行补了个长按输出alt+F4,给win环境下单手关机用.
- 编辑于202604：由于nav数字换家了，数字除了左右互换，变化不大。然后alt f4的tap dance在两只手都放了。都可以单手关机了。此外，由于ZMK可以支持绑定5个BT设备，原来的方案只留了3个绑定槽位实在有点浪费，正好数字层左手顶部是空的，就把BT区都放上去了。
- media layer:
- <img width="1231" height="359" alt="media" src="https://github.com/user-attachments/assets/77d9a947-721e-478a-af6c-e67462b7f59a" />
- 进media layer的方式定义了一个tap dance,单击是momentary瞬切层,用于即用即抛的情况;双击是to静态切层,主要给媒体键用.键位分布~~左手~~ 右手是同num层映射的Fn层,~~右手~~ 左手是媒体键,复用homerow mod配合Fn使用. 为了避免误触,在边角的地方安排了reset, bootloader, ~~bt clear, bt 0和bt 1~~ 这些键盘控制键;最后找了空余的地方放了RGB控制键.
- 总结:
...
- 还有进步的空间吗?之后还会有啥可能得变化?
- >ZMK下,mouse move并没有完成开发,所以现在的ZMK键盘只能控制光标,不能模拟鼠标移动.以后如果鼠标移动可以支持了,马上会加入mouse层,这可太重要了.当然更快的选择是加轨迹球.国内已经有好多大神做出来了,我也会找机会试.
- 编辑于202604：mousemove不仅有了，还加入了十字键来控制mouse。mousemove复用nav的键位，基本1：1复刻，简直太棒了！有种逻辑闭环的美感。
- <img width="1230" height="358" alt="mouse" src="https://github.com/user-attachments/assets/07904727-1099-43bd-a341-b8967e2b7e81" />
- 此外:
- 除了Keymap,还有什么能帮助我们更进一步提高速度吗?有的，兄弟，有的. 键盘布局.对编程用户,英文输入用户来说,QWER改Colemak Mod-DH才是出路.但就像中文输入法改五笔,改双拼一样,键盘布局的学习曲线会更陡峭一点,看各人的需求和选择了.对我而言,QWER+双拼已经是很好的选择了,而且双拼就是基于QWER下对中文的优化,在Colemak下反而不能正常工作. 当然双拼也会带来点麻烦,就是你离不开自己的电脑/键盘,以及别人用不了你的电脑/键盘.啊,这似乎是个优点...
